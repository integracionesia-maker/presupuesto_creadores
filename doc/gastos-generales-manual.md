...
> Feature del paquete R12 (`docs/plan-gastos-generales-y-borrado-tickets.md`). Complementa `doc/presupuestos-y-validacion.md` (que cubre presupuestos de creadores) — los Gastos Generales son un dominio **independiente**.

## ¿Qué son?

Gastos operativos que **no** están ligados a un creador ni a una marca (suscripciones de software, servicios, herramientas internas, etc.). Por eso viven en su propia tabla (`general_expenses`) en vez de reutilizar `tickets`:
...
La tabla principal lista los gastos generales (más recientes primero), con un filtro de rango de fechas. Cada fila muestra fecha, descripción, monto y un botón **"Ver"** para abrir el comprobante en una pestaña nueva.

## Exportar a PDF

1. Botón **"Exportar"**.
2. Selecciona uno o más de los últimos 12 meses (checkboxes).
3. **"Generar PDF"** produce un reporte con: título, período seleccionado, tabla (Fecha | Descripción | Monto | Comprobante), total general, y pie de página con fecha de generación y quién lo generó.
