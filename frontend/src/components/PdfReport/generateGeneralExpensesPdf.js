/**
 * Orquesta la captura de la plantilla off-screen (#general-expenses-pdf-root,
 * ver GeneralExpensesPdfTemplate) sección por sección con html2canvas y las
 * ensambla como páginas en jsPDF. Ambas librerías se cargan con import()
 * dinámico para no engordar el bundle principal — solo se descargan al hacer
 * click en "Descargar PDF".
 */
export async function generateGeneralExpensesPdf({ filename }) {
  const [{ default: jsPDF }, { default: html2canvas }] = await Promise.all([
    import("jspdf"),
    import("html2canvas"),
  ]);

  const root = document.getElementById("general-expenses-pdf-root");
  if (!root) {
  ...
  pdf.save(filename || "reporte-gastos-generales.pdf");
}
